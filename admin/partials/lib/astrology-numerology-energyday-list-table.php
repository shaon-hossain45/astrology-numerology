 ...
 */
class Custom_List_Table_EnergyDay extends WP_List_Table {
	...
	function __construct() {
		...
		parent::__construct(
			array(
				'singular' => 'astrologynumerologyenergyday',
				'plural'   => 'astrologynumerologyenergydays',
			)
		);
	}
	...
	function column_template_title( $item ) {
		...
		$delete_query_args = array(
			...
			'action' => 'deleteenergyday',
			'id'     => $item['ID'],
		);
		$actions           = array(
			//'edit'   => sprintf( '<a href="?page=astrology_numerology&action=edit&id=%s">%s</a>', $item['ID'], __( 'Edit', 'cltd_example' ) ),
			'delete' => sprintf( '<a href="%1$s">%2$s</a>', esc_url( wp_nonce_url( add_query_arg( $delete_query_args, 'admin.php' ), 'deleteenergyday' ) ), _x( 'Delete', 'List table row action', 'wp-list-table-example' ) ),
		);
	...
	public function no_items() {
		_e( 'No wpap numerology items found.' );
	}
	/**
	...
	public function process_bulk_action() {
		...
		if ( 'deleteenergyday' === $this->current_action() ) {
			// In our file that handles the request, verify the nonce.
			$nonce = esc_attr( $_REQUEST['_wpnonce'] );

			if ( ! wp_verify_nonce( $nonce, 'deleteenergyday' ) ) {
		...
		if ( 'bulk-deleteenergyday' === $this->current_action() ) {
			...
			if ( ! empty( $delete_ids ) ) {
				...
				if ( $counter > 1 ) {
					$counter = $counter . ' wpapnumerology items';
				} else {
					$counter = $counter . ' wpapnumerology item';
				}
