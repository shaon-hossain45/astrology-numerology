 ...
 */
class Custom_List_Table_PowerMercuryMonth extends WP_List_Table {
	...
	function __construct() {
		...
		parent::__construct(
			array(
				'singular' => 'astrologynumerologypowermercurymonth',
				'plural'   => 'astrologynumerologypowermercurymonths',
			)
		);
	}
	...
	function column_template_title( $item ) {
		...
		$delete_query_args = array(
			...
			'action' => 'deletepowermercurymonth',
			'id'     => $item['ID'],
		);
		$actions           = array(
			'edit'   => sprintf( '<a href="?page=astrology_numerology&action=edit&id=%s">%s</a>', $item['ID'], __( 'Edit', 'cltd_example' ) ),
			'delete' => sprintf( '<a href="%1$s">%2$s</a>', esc_url( wp_nonce_url( add_query_arg( $delete_query_args, 'admin.php' ), 'deletepowermercurymonth' ) ), _x( 'Delete', 'List table row action', 'wp-list-table-example' ) ),
		);
	...
	public function no_items() {
		_e( 'No astrology numerology power mercury month items found.' );
	}
	/**
	...
	public static function itechpublic_delete_activity( $data ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'astrology_numerology_powermercurymonth';
	...
	function get_bulk_actions() {
		$actions = array(
			'bulk-deletepowermercurymonth'         => 'Delete',
			//'bulk-edit' => 'Edit',
		);
		return $actions;
	}
	...
	public function process_bulk_action() {
		...
		if ( 'deletepowermercurymonth' === $this->current_action() ) {
			// In our file that handles the request, verify the nonce.
			$nonce = esc_attr( $_REQUEST['_wpnonce'] );

			if ( ! wp_verify_nonce( $nonce, 'deletepowermercurymonth' ) ) {
				die( 'Go get a life script kiddies' );
			} else {
				self::itechpublic_delete_activity( absint( $_REQUEST['id'] ) );

				//add_flash_notice( __( '1 wpapnumerology item permanently deleted.' ), 'success', true );
		...
		if ( 'bulk-deletepowermercurymonth' === $this->current_action() ) {
			...
			if ( ! empty( $delete_ids ) ) {
				...
				if ( $counter > 1 ) {
					$counter = $counter . ' astrology numerology power mercury month items';
				} else {
					$counter = $counter . ' astrology numerology power mercury month item';
				}
				...
				$redirect_to = admin_url( 'admin.php?page=astrology_numerology&deleted=false' );
				wp_redirect( $redirect_to );
				exit;
			}
		}
