 ...
 */
class Custom_List_Table_Year extends WP_List_Table {
	...
	function __construct() {
		...
		parent::__construct(
			array(
				'singular' => 'astrologynumerologyyear',
				'plural'   => 'astrologynumerologyyears',
			)
		);
	}
	...
	function column_template_title( $item ) {
		...
		$actions           = array(
			...
			'delete' => sprintf( '<a href="%1$s">%2$s</a>', esc_url( wp_nonce_url( add_query_arg( $delete_query_args, 'admin.php' ), 'deletewpapnumerology' ) ), _x( 'Delete', 'List table row action', 'wp-list-table-example' ) ),
		);
	...
	public function no_items() {
		_e( 'No astrology numerology year numerology items found.' );
	}
	/**
	...
	public static function itechpublic_delete_activity( $data ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'astrology_numerology_year';
	...
	protected function get_views() {
		...
		$views_item   = array(
			'template_title' => array(
				'name'      => __( 'numerology Title', 'cltd_example' ),
				'status_id' => 1,
			),
			'template_description' => array(
				'name'      => __( 'numerology Description', 'cltd_example' ),
				'status_id' => 2,
			),
		);
	...
	public function process_bulk_action() {
		...
		if ( 'deleteyear' === $this->current_action() ) {
			// In our file that handles the request, verify the nonce.
			$nonce = esc_attr( $_REQUEST['_wpnonce'] );

			if ( ! wp_verify_nonce( $nonce, 'deletewpapnumerology' ) ) {
		...
		if ( 'bulk-deleteyear' === $this->current_action() ) {
			...
			if ( ! empty( $delete_ids ) ) {
				...
				if ( $counter > 1 ) {
					$counter = $counter . ' astrology numerology year items';
				} else {
					$counter = $counter . ' astrology numerology year item';
				}
