 ...
 */
class Custom_List_Table_Month extends WP_List_Table {
	...
	function __construct() {
		...
		parent::__construct(
			array(
				'singular' => 'astrologynumerologymonth',
				'plural'   => 'astrologynumerologymonths',
			)
		);
	}
	...
	public function no_items() {
		_e( 'No astrology numerology month items found.' );
	}
	/**
	...
	public static function itechpublic_delete_activity( $data ) {
		global $wpdb;
		$table_name = $wpdb->prefix . 'astrology_numerology_month';
	...
	public function process_bulk_action() {
		...
		if ( 'deletemonth' === $this->current_action() ) {
			// In our file that handles the request, verify the nonce.
			$nonce = esc_attr( $_REQUEST['_wpnonce'] );

			if ( ! wp_verify_nonce( $nonce, 'deletemonth' ) ) {
				die( 'Go get a life script kiddies' );
			} else {
				self::itechpublic_delete_activity( absint( $_REQUEST['id'] ) );

				//add_flash_notice( __( '1 astrology numerology month item permanently deleted.' ), 'success', true );
		...
		if ( 'bulk-deletemonth' === $this->current_action() ) {
			...
			if ( ! empty( $delete_ids ) ) {
				...
				if ( $counter > 1 ) {
					$counter = $counter . ' astrology numerology month items';
				} else {
					$counter = $counter . ' astrology numerology month item';
				}
